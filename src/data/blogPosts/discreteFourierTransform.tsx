...
export default function DiscreteFourierTransformPost() {
  return (
    <article className="blog-article">
      ...
      <p>
        So one length-<InlineMath tex="N/2"/> DFT of the even-indexed samples, plus one length-<InlineMath tex="N/2"/> DFT of the odd-indexed samples, plus a linear-time combine, gives the entire length-<InlineMath tex="N"/> DFT. The recurrence <InlineMath tex="T(N) = 2T(N/2) + O(N)"/> resolves to <strong><InlineMath tex="O(N \log N)"/></strong>.
      </p>
      <p>
      ...
      <p>
        Think of it as a graph problem. Build a 6-node graph <InlineMath tex="G_p"/> whose edges are exactly the unordered letter pairs <InlineMath tex="\{a, b\}"/> that co-occur at alignment <InlineMath tex="p"/> — i.e. some offset <InlineMath tex="i"/> has <InlineMath tex="s_{p+i} = a"/> and <InlineMath tex="t_i = b"/>. The cost at <InlineMath tex="p"/> is the number of merges we need to perform. Each connected component of size <InlineMath tex="\ell"/> needs <em>exactly</em> <InlineMath tex="\ell - 1"/> merges to collapse to a single equivalence class.
      </p>
      <p>
      ...
      <p>
        Human eyes are a lot <em>more sensitive to low frequencies than to high frequencies</em>. After the DCT is applied to a block, each cell is quantised by dividing by a quantisation factor that is <em>larger for higher frequencies</em>, so that many of the high-frequency cells <em>become zero</em> and can be efficiently compressed with <em>run-length encoding</em> when stored in a <em>zig-zag pattern</em> from the top-left corner. The DCT is basically the same as the DFT but with <em>real-valued cosine probes</em> instead of complex exponentials, and it also has a fast algorithm like the FFT. Often an image can be reduced by <strong>90% in size with barely any perceptible loss in quality</strong>, just by throwing away the high-frequency DCT coefficients.
      </p>
      <p>
        Another example in image processing is <strong>deblurring an image by deconvolving it with the blur kernel in the frequency domain</strong>. Blurring an image can often be modelled as a convolution of the original image with a <em>blur kernel</em>, so by the convolution theorem we can <em>undo the blur</em> by dividing the DFT of the blurred image by the DFT of the kernel, and then inverse-DFT back to recover the deblurred image. This is a common technique in <em>computational photography</em>, but the practical details are a bit more complicated — there is the need for a <em>window function</em> to avoid <em>frequency leakage</em>, and the fact that the kernel's DFT may have small values that cause <em>numerical instability</em> when dividing.
      </p>
      <h3>Cryptography</h3>
      <p>
        The number-theoretic transform can be used to <strong>efficiently multiply polynomials over finite fields</strong>, which is a key operation in some <em>post-quantum cryptographic</em> schemes. The general DFT can also speed up multiplication of <em>very large integers</em> by treating them as polynomials in a base and using the convolution theorem. This is used in some implementations of <em>RSA</em> and other cryptosystems to speed up <em>key generation</em> and <em>encryption/decryption</em> operations.
      </p>
      ...
      <p>
        where the <em>SSM kernel</em> is <InlineMath tex="K_n = \bar C\, \bar A^{\,n-1}\, \bar B"/> for <InlineMath tex="n \ge 1"/>. The earlier <InlineMath tex="D x_n"/> feedthrough re-enters as a residual stream around this convolution, exactly as in any <em>ResNet</em> block. So the entire sequence-to-sequence map of a linear time-invariant SSM is a single convolution of the input against one fixed kernel — <strong>the recurrent form and the convolutional form are two views of the same operator</strong>.
      </p>
      <p>
        This is where the convolution theorem earns its keep. The recurrence is inherently sequential: you cannot compute <InlineMath tex="h_{n+1}"/> until <InlineMath tex="h_n"/> is done, which wastes the parallelism a GPU is built for. Naive convolution is <InlineMath tex="O(L^2)"/> in the sequence length <InlineMath tex="L"/> — the same ballpark as a <em>transformer's attention</em>. FFT-based convolution drops it to <InlineMath tex="O(L \log L)"/> with <em>every stage trivially parallel</em>. So during training you precompute <InlineMath tex="K"/>, FFT both <InlineMath tex="K"/> and <InlineMath tex="x"/>, multiply pointwise, inverse-FFT back; at inference you switch to the recurrence and pay <InlineMath tex="O(d^2)"/> per step with no growing KV cache. <strong>That duality is what makes modern long-context SSMs like S4 and the Mamba lineage tractable.</strong>
      </p>
